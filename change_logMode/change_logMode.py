...
#["SECURITY_RULE_LOG_MODE_NO_LOG", "SECURITY_RULE_LOG_MODE_AT_SESSION_START", "SECURITY_RULE_LOG_MODE_AT_SESSION_END", "SECURITY_RULE_LOG_MODE_AT_RULE_HIT", "SECURITY_RULE_LOG_MODE_AT_SESSION_START_AND_END"]

    for item in data['items']:
        if 'SECURITY_RULE_LOG_MODE_AT_RULE_HIT' !=  item['logMode']:  # ЕСЛИ НЕ HIT
            payload = {
                "id": item['id'],
                "name": item['name'],
                ...
                "logMode": "SECURITY_RULE_LOG_MODE_AT_RULE_HIT",   # СЮДА ПИШИ НА ЧТО МЕНЯТЬ!! 
            ...
            if response.status_code == 200:
                print(f"{item['name']}: log_mode_old {item['logMode']}")
            else:
